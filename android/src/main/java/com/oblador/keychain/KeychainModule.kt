...
class KeychainModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    @StringDef(AccessControl.NONE, AccessControl.USER_PRESENCE, AccessControl.BIOMETRY_ANY, AccessControl.BIOMETRY_CURRENT_SET, AccessControl.DEVICE_PASSCODE, AccessControl.APPLICATION_PASSWORD, AccessControl.BIOMETRY_ANY_OR_DEVICE_PASSCODE, AccessControl.BIOMETRY_CURRENT_SET_OR_DEVICE_PASSCODE)
    ...
    private fun internalWarmingBestCipher() {
        try {
            ...
            val requiredLevel = if (isSecure) SecurityLevel.SECURE_HARDWARE else SecurityLevel.SECURE_SOFTWARE
    ...
    protected fun setGenericPassword(alias: String,
                                     username: String,
                                     password: String,
                                     options: ReadableMap?,
                                     promise: Promise) {
    ...
    @ReactMethod
    fun setGenericPasswordForOptions(options: ReadableMap?,
                                     username: String,
                                     password: String,
                                     promise: Promise) {
    ...
    protected fun getGenericPassword(alias: String,
                                     options: ReadableMap?,
                                     promise: Promise) {
        try {
            ...
            cipher = if (rules == Rules.AUTOMATIC_UPGRADE && storageName == KnownCiphers.FB) {
    ...
    @ReactMethod
    fun getGenericPasswordForOptions(options: ReadableMap?,
                                     promise: Promise) {
        val service = getServiceOrDefault(options)
        getGenericPassword(service, options, promise)
    }

    protected fun resetGenericPassword(alias: String,
                                       promise: Promise) {
        try {
    ...
    @ReactMethod
    fun resetGenericPasswordForOptions(options: ReadableMap?,
                                       promise: Promise) {
        val service = getServiceOrDefault(options)
    ...
    @ReactMethod
    fun hasInternetCredentialsForServer(server: String,
                                        promise: Promise) {
    ...
    @ReactMethod
    fun hasGenericPasswordForOptions(options: ReadableMap?,
                                     promise: Promise) {
        val service = getServiceOrDefault(options)
    ...
    @ReactMethod
    fun setInternetCredentialsForServer(server: String,
                                        username: String,
                                        password: String,
                                        options: ReadableMap?,
                                        promise: Promise) {
    ...
    @ReactMethod
    fun getInternetCredentialsForServer(server: String,
                                        options: ReadableMap?,
                                        promise: Promise) {
    ...
    @ReactMethod
    fun resetInternetCredentialsForServer(server: String,
                                          promise: Promise) {
    ...
    @ReactMethod
    fun getSecurityLevel(options: ReadableMap?,
                         promise: Promise) {
    ...
    private fun decryptCredentials(alias: String,
                                   current: CipherStorage,
                                   resultSet: PrefsStorage.ResultSet,
                                   @Rules rules: String,
                                   promptInfo: PromptInfo): DecryptionResult {
        val storageName = resultSet.cipherStorageName
        ...
        val oldStorage = getCipherStorageByName(storageName)
                ?: throw KeyStoreAccessException("Wrong cipher storage name '$storageName' or cipher not available")
    ...
    @Throws(CryptoFailedException::class)
    private fun decryptToResult(alias: String,
    ...
    protected fun getInteractiveHandler(current: CipherStorage, promptInfo: PromptInfo): DecryptionResultHandler {
        val reactContext = reactApplicationContext
        return DecryptionResultHandlerProvider.getHandler(reactContext, current, promptInfo)
    }

    /** Remove key from old storage and add it to the new storage.  */ /* package */
    @Throws(KeyStoreAccessException::class, CryptoFailedException::class, IllegalArgumentException::class)
    fun migrateCipherStorage(service: String,
                             newCipherStorage: CipherStorage,
                             oldCipherStorage: CipherStorage,
                             decryptionResult: DecryptionResult) {

      val username = decryptionResult.username ?: throw IllegalArgumentException("Username cannot be null")
      val password = decryptionResult.password ?: throw IllegalArgumentException("Password cannot be null")
    ...
    fun  /* package */getCipherStorageForCurrentAPILevel(useBiometry: Boolean): CipherStorage {
        ...
        val isBiometry = useBiometry && (isFingerprintAuthAvailable || isFaceAuthAvailable || isIrisAuthAvailable)
    ...
    val isFingerprintAuthAvailable: Boolean
        ...
        get() = DeviceAvailability.isStrongBiometricAuthAvailable(reactApplicationContext) && DeviceAvailability.isFingerprintAuthAvailable(reactApplicationContext)
    val isFaceAuthAvailable: Boolean
        /** True - if face recognition hardware available and configured, otherwise false.  */
        get() = DeviceAvailability.isStrongBiometricAuthAvailable(reactApplicationContext) && DeviceAvailability.isFaceAuthAvailable(reactApplicationContext)
    val isIrisAuthAvailable: Boolean
        /** True - if iris recognition hardware available and configured, otherwise false.  */
        get() = DeviceAvailability.isStrongBiometricAuthAvailable(reactApplicationContext) && DeviceAvailability.isIrisAuthAvailable(reactApplicationContext)
    ...
    companion object {
        ...
        @Rules
        private fun getSecurityRulesOrDefault(options: ReadableMap?,
        ...
        @AccessControl
        private fun getAccessControlOrDefault(options: ReadableMap?,
                                              @AccessControl fallback: String): String {
        ...
        private fun getSecurityLevelOrDefault(options: ReadableMap?,
                                              fallback: String): SecurityLevel {
        ...
        fun getUseBiometry(@AccessControl accessControl: String?): Boolean {
            return AccessControl.BIOMETRY_ANY == accessControl || AccessControl.BIOMETRY_CURRENT_SET == accessControl || AccessControl.BIOMETRY_ANY_OR_DEVICE_PASSCODE == accessControl || AccessControl.BIOMETRY_CURRENT_SET_OR_DEVICE_PASSCODE == accessControl
        }

        /** Extract user specified prompt info from options.  */
        private fun getPromptInfo(options: ReadableMap?): PromptInfo {
            val promptInfoOptionsMap = if (options != null && options.hasKey(Maps.AUTH_PROMPT)) options.getMap(Maps.AUTH_PROMPT) else null
            ...
            /* PromptInfo is only used in Biometric-enabled RSA storage and can only be unlocked by a strong biometric */promptInfoBuilder.setAllowedAuthenticators(BiometricManager.Authenticators.BIOMETRIC_STRONG)

            /* Bypass confirmation to avoid KeyStore unlock timeout being exceeded when using passive biometrics */promptInfoBuilder.setConfirmationRequired(false)
            return promptInfoBuilder.build()
        }

        /** Throw exception in case of empty credentials providing.  */
        @Throws(EmptyParameterException::class)
        fun throwIfEmptyLoginPassword(username: String?,
                                      password: String?) {
        ...
        @Throws(CryptoFailedException::class)
        fun throwIfInsufficientLevel(storage: CipherStorage,
                                     level: SecurityLevel) {
            if (storage.securityLevel().satisfiesSafetyThreshold(level)) {
                return
            }
            throw CryptoFailedException(String.format(
